...
const rdfPrefixStr = rdfPrefixes.reduce(
  (acc, { prefix, uri }) => `${acc} PREFIX ${prefix}: <${uri}>\n`,
  '',
)

const applyQueryTemplate = (
  template: string,
  variables: QueryVariables,
): string => {
  const query = Object.entries(variables).reduce(
    (q, [name, value]) =>
      q.replace(new RegExp(`%${name}%`, 'g'), ((value: any): string)),
    template,
  )
  return `${rdfPrefixStr}\n${query}`
}
...
const runSparqlQuery = (
  ...
  return fetchWithTimeout(uri, { method: 'GET' }, 20000).then(
    (resp: Response): { [string]: any } => {
